export default {
	...
	getAllDrives() {
		...
		const getEntryText = (parentEl, tagName) => {
			if (!parentEl) return "";
			const children = Array.from(parentEl.children);
			const found = children.find(el => el.tagName === "entry" && el.getAttribute("name") === tagName);
			return found ? found.textContent.trim() : "";
		};

		const getDirectEntriesBlock = (parentEl, sectionName) => {
			if (!parentEl) return null;
			const children = Array.from(parentEl.children);
			return children.find(el => el.tagName === "entries" && el.getAttribute("name") === sectionName) || null;
		};

		const allEntriesBlocks = Array.from(doc.querySelectorAll('entries[name="disk"]'));
		const diskBlocks = allEntriesBlocks.filter(el => {
			const parent = el.parentElement;
			return !parent || parent.tagName !== "entries" || parent.getAttribute("name") !== "target";
		});

		const erasureBlocks = Array.from(doc.querySelectorAll('entries[name="erasure"]'));
		const erasureBySerial = {};

		erasureBlocks.forEach(block => {
			const targetBlock = getDirectEntriesBlock(block, "target");
			const serial = getEntryText(targetBlock, "serial");
			if (!serial) return;

			const record = {
				state: getEntryText(block, "state"),
				erasure_standard_name: getEntryText(block, "erasure_standard_name"),
				start_time: getEntryText(block, "start_time"),
				end_time: getEntryText(block, "end_time")
			};

			if (!erasureBySerial[serial]) erasureBySerial[serial] = [];
			erasureBySerial[serial].push(record);
		});

		const pickBestErasure = (serial) => {
			const records = erasureBySerial[serial] || [];
			const success = records.find(r => /success|erased/i.test(r.state));
			return success || records[0] || {};
		};

		return diskBlocks.map((diskEl, i) => {
			const serial = getEntryText(diskEl, "serial");
			const capacityBytes = getEntryText(diskEl, "capacity");
			const best = pickBestErasure(serial);

			return {
				disk_index: i + 1,
				model: getEntryText(diskEl, "model"),
				serial: serial,
				interface_type: getEntryText(diskEl, "interface_type"),
				capacity: this.bytesToGBDecimal(capacityBytes),
				health: getEntryText(diskEl, "health_status") || getEntryText(diskEl, "health"),
				power_on_hours: getEntryText(diskEl, "power_on_hours"),
				erasure_status: best.state || "",
				erasure_standard: best.erasure_standard_name || "",
				erasure_start_time: best.start_time || "",
				erasure_end_time: best.end_time || ""
			};
		});
	},
	...
	async fetchAndSave() {
		...
		try {
			await qry_set_data_erasure_status.run();
		} catch(e) {
			showAlert('Status update failed: ' + e.message, 'warning');
		}
		...
		const allDrives = this.getAllDrives();
		if (allDrives.length) {
			try {
				await qry_insert_drives.run({ drives: allDrives });
			} catch(e) {
				showAlert('Drive table write failed: ' + e.message, 'warning');
			}
		}
